...
if __name__ == "__main__":
    if path_folder_data.exists():
        ...
        #Create view 
        spark.sql(f"""CREATE OR REPLACE GLOBAL TEMP VIEW us_origin_airport_SFO_global_tmp_view AS
        SELECT date, delay, origin, destination from us_delay_flights_tbl WHERE 
        origin = 'SFO';""")
        
        spark.sql(f"""SELECT * FROM global_temp.us_origin_airport_SFO_global_tmp_view""")

        spark.sql(f"""DROP VIEW IF EXISTS us_origin_airport_SFO_global_tmp_view;""")


        #OR
        spark.sql(f"""CREATE OR REPLACE TEMP VIEW us_origin_airport_JFK_tmp_view AS
        SELECT date, delay, origin, destination from us_delay_flights_tbl WHERE 
        origin = 'JFK'""")
