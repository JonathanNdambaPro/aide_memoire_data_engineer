...
def _pick_erp_system(**context):
    """
    Renvoie un task id pour que le BranchPythonOperator puisse choisir
    lequel utiliser
    """
...
with DAG(
    ...
    join_datasets = DummyOperator(task_id="join_datasets", trigger_rule="none_failed") #Changement de trigger_rule car preceder d'un BranchPythonOperator
    train_model = DummyOperator(task_id="train_model")
    deploy_model = DummyOperator(task_id="deploy_model")

    start >> [pick_erp_system, fetch_weather]
    pick_erp_system >> [fetch_sales_old, fetch_sales_new] #Cense etre un traitement parallel mais avec BranchPythonOperator de vieens un choix, ne pas oublier de changer le Triggr_rule comme les deux task n seront jamais simultanement vraie
